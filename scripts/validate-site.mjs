...
function validateTarget(source, raw) {
  ...
  // Poster print masters (/posters/*.png|pdf) are intentionally not hosted — the site
  // displays WebP previews and downloads were removed. Don't require the masters on disk.
  // Previews (/posters/previews/*.webp) are still validated normally below.
  if (/^\/posters\/[^/]+\.(?:png|pdf)$/i.test(clean)) return;
  let decoded;
