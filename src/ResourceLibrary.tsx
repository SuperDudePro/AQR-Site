...
const coreResources = [
  ...
  { title: "2026–27 Course Syllabus", kind: "Course information", href: "https://appliedquantitativereasoning.com/syllabus.html", summary: "Student-facing AQR syllabus with grading, Weekly Proof, technology, attendance, tardy, cell phone, revision, materials, and course expectations." },
  ...
  { title: "AQR Vocabulary", kind: "Student resource", href: "/vocabulary", summary: "Core AQR language and Quarter 1 through Quarter 4 vocabulary sections using one consistent entry format." },
  { title: "Quarter 1 — Know Yourself", kind: "Quarter", href: "/quarter-1", summary: "Learner profile, personalized support, measurement, misleading claims, and revision." },
  ...
  { title: "Quarter 3 — Build a Decision Tool", kind: "Quarter", href: "/quarter-3", summary: "Criteria, tradeoffs, assumptions, weighting, risk, cost, uncertainty, and testing." },
    ...
    <div className="poster-site-shell">
      ...
      <main className="poster-page" id="resource-main-content">
        ...
        <section className="poster-section poster-section-silver" aria-labelledby="resource-results-title"><div className="poster-wrap"><div className="poster-section-head poster-section-head-dark"><p className="poster-section-kicker">Search and filter</p><h2 id="resource-results-title">{matches.length} matching resource{matches.length === 1 ? "" : "s"}</h2></div>
          ...
          <ul className="poster-type-grid" role="list">{matches.map((resource) => <li className="poster-type-list-item" key={`${resource.kind}-${resource.title}-${resource.href}`}><a className="poster-type-card" href={resource.href} download={resource.kind === "Poster" ? true : undefined}><div className="poster-type-copy"><p className="poster-card-kicker">{resource.kind}</p><h3>{resource.title}</h3><p>{resource.summary}</p><span className="poster-card-link">{resource.kind === "Poster" ? "Download PDF" : "Open resource"}</span></div></a></li>)}</ul>
        </div></section>
      </main>
